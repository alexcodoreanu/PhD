pro split_artificial_mg2
	...
	for z = 0, n_elements(pixels) - 1 do begin
		...
		for index = 0, 3 do begin
			...
			input_name =  fits_path + "unbinned_artificial_" + batch_index + "_" + index_string + ".fits"
			unbnned_file = mrdfits(input_name, 1)
			
			split_file = replicate(str, 128.00*n_elements(unbnned_file.ew))
			output_name = fits_path + "split_artificial_" + batch_index + "_" + index_string + ".fits"
