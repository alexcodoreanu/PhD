...
;	close, /all
  ...
  redshift_min = 4.91
	...
	output_fits_name = fits_output_path +  'si4_single_profiles_' + cd_string + '.fits'
		...
		cd_string = cd_string1 + 'd' + cd_string2
			...
			for b = 0, n_elements(b_range) - 1 do begin
				...
				for z = 0,  n_elements(redshift) - 1 do begin
					...
					output_file =  'si4_' + cd_string + '_' + b_string + '_' + z_string + '.dat'
			
					lambda_min = (redshift[z] + 1.0)*1370.0
					lambda_max = (redshift[z] + 1.0)*1410.0
					
					;	create temp.f26 file
					temp_file = 'temp_si4_' + cd_string + '.f26'
					profile_template = 'profile_template_si4_' + cd_index_string + '.dat'
