...
;
pro combine_human_acceptance_to_real_si4, cd_index
...
input_fits_name = fits_input_path + 'real_si4_human_acc_fail_' + cd_index_string + '.fits'
...
for s = 0, n_elements(SOLS) - 1 do begin
	SOL = SOLS[s]
	sol_result_name = fits_results_path + 'si4_comp_sol_' + SOL + '_' + cd_index_string + '.fits'
	...
	rs = finite(sol_input.USER_SUCCESS_1402)
	sol_input[where(rs lt 1)].USER_SUCCESS_1402 = 0.0
	
	rs = finite(sol_input.USER_FAILURE_1402)
	sol_input[where(rs lt 1)].USER_FAILURE_1402 = 0.0
	
	human_acc_result_name = fits_results_path + 'si4_h_acc_sol_' + SOL + '_' + cd_index_string + '.fits'
