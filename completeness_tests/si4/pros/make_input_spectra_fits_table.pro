pro make_input_spectra_fits_table, z_index, SOL, local_element
	...
	template_file = SOL + '_si4_' + z_index_string + '.dat'
	...
	temp = SOL + '_si4_' + z_index_string + '.f26'
  ...
  column_density_range = dindgen(16)/10 + 12.2
	...
	redshift_min = 4.91
	redshift = redshift_min + z_index*0.001 + dindgen(25)*0.05
	
	redshift_error = 0.00001
	
	str = {column_density:0.0, b:0.0, redshift:fltarr(25), flux:fltarr(n_elements(f))}
	structure = replicate(str, n_elements(column_density_range)*n_elements(b_range)*2)	
	output_name = fits_path + 'si4_' + SOL + '_z_index_' + z_index_string + '.fits'
	...
	for c = 0, n_elements(column_density_range) - 1 do begin
		...
		for b = 0, n_elements(b_range) - 1 do begin			
			...
			for z = 0, n_elements(redshift) - 1 do begin
				...
				lambda_min = (redshift[z] + 1.0)*1380.
				lambda_max = (redshift[z] + 1.0)*1420.
				;	create temp.f26 file
					openw, lun, temp, /get_lun
						...
						if (redshift[z] + 1)*1393.76018 gt 13000 then $
						line = '%% ' + template_file + '    1     ' + $
							    string(lambda_min) + '      ' + $
					...
					output_file = 'si4_' + SOL + '_'+ cd_string + '_' + b_string + '_' + z_string + '.dat'
