pro make_si4_recovery_fits
	...
	for s = 0, n_elements(sightline) - 1 do begin
		SOL = sightline[s]		
		input_file_name = input_path + 'r_si4_' + SOL + '_z_index_0.fits'
		
		sol_fits = mrdfits(input_file_name, 1, /SILENT)
		
		output_fits_name = input_path + 'r_si4_' + SOL + '_all.fits'
		...
		for z = 1, 50 do begin
			...
			output_file_name = input_path + 'r_si4_' + SOL + '_z_index_' + z_index_string + '.fits'
		...
		for i = 0, n_elements(sol_fits.b) - 1 do begin
			for a = 0, n_elements(sol_fits[i].redshift) - 1 do begin
				...
				output_file[counter].lambda         = (1. + sol_fits[i].redshift[a])*si4_1393
