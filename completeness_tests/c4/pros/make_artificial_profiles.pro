...
for c = 0, n_elements(column_density_range) - 1 do begin
	...
	input_fits_name  = fits_output_path + 'c4_single_profiles_' + cd_index_string + '.fits'
	output_fits_name = fits_output_path + 'c4_artificial_single_profiles_' + cd_index_string + '.fits'
	...
	for i = 0, n_elements(original_fits.z) - 1 do begin
		...
		min_art_lambda = double(lambda[doublet_index[doublet_index_break+1]]/c4_1550*c4_art)
	...
	;	stop
		...
		output_fits[i].vp    = artificial_vpf[0:n_elements(output_fits[i].vp)-1]
		output_fits[i].z     = original_fits[i].z  
		
		print, c, i, n_elements(original_fits.z) - 1
