pro get_bar_values_art_c4
	...
	;	get real failure
	failure_path = '/lustre/projects/p036_swin/alexc/completeness_test/c4/fits/'
	s = 0
	sol = sightline[s]
	failure_fits_name = failure_path + sol + '_art_failure_c_cd0d1_z0d01.fits'
  failure_fits = mrdfits(failure_fits_name, 1, /silent)
	...
	for z = 0, n_elements(zmin) - 1 do begin
		...
		print, zmin[z], zmax[z], total(failure_fits[zindex].USER_FAILURE_1548)/n_elements(zindex)
		
	endfor

	;	get cbar
	cbar_path = '/lustre/projects/p036_swin/alexc/completeness_test/c4/fits/results/'
	s = 0
	sol = sightline[s]
	cbar_fits_name = cbar_path + sol + '_art_c4_h_acc_cd0d1_z0d01.fits'
  cbar_fits = mrdfits(cbar_fits_name, 1, /silent)
	...
	;	get lbar
	lbar_path = '/lustre/projects/p036_swin/alexc/completeness_test/c4/fits/results/'
	s = 0
	sol = sightline[s]
	lbar_fits_name = lbar_path + sol + '_art_binned_cd0d1_z0d01.fits'
