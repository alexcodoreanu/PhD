...
;
pro combine_human_acceptance_to_real_c4, cd_index
...
input_fits_name = fits_input_path + 'real_c4_human_acc_fail_' + cd_index_string + '.fits'
...
for s = 0, n_elements(SOLS) - 1 do begin
	SOL = SOLS[s]
	sol_result_name = fits_results_path + 'c4_comp_sol_' + SOL + '_' + cd_index_string + '.fits'
	...
	rs = finite(sol_input.USER_SUCCESS_1550)
	sol_input[where(rs lt 1)].USER_SUCCESS_1550 = 0.0
	
	rs = finite(sol_input.USER_FAILURE_1550)
	sol_input[where(rs lt 1)].USER_FAILURE_1550 = 0.0
	
	human_acc_result_name = fits_results_path + 'c4_h_acc_sol_' + SOL + '_' + cd_index_string + '.fits'
