...
function select_mg2_systems, lambda, flux, error, minimum, maximum, structure_name
	...
	for ind = 0, n_elements(structure.REDSHIFT) - 1 do begin
		;	get the pixel location in the lambda array
			local_index_2796 = findel(structure[ind].X_MIN_2796, lambda)
			absorption_feature_2796 =uint(local_index_2796  + dindgen(structure[ind].NUM_PIXELS ))

			local_index_2803 = findel(structure[ind].X_MIN_2796*2803.5314853/2796.3542699, lambda)
			absorption_feature_2803 =uint(local_index_2803  + dindgen(structure[ind].NUM_PIXELS ))
			
			;print, ind
			
		; check that the ratio criteria holds for each pixel 
			check = 0.0
			for p = 0, n_elements(absorption_feature_2796) - 1 do begin
				numerator = (1.0 - flux[absorption_feature_2796[p]])
				if numerator lt 0.0 then numerator = flux[absorption_feature_2796[p]]
			
				denumerator = (1.0 - flux[absorption_feature_2803[p]])
				if denumerator lt 0.0 then denumerator = flux[absorption_feature_2803[p]]
			
				error_value = sqrt((1./denumerator/denumerator)*(error[absorption_feature_2796[p]]*error[absorption_feature_2796[p]])* $
											(numerator*numerator/denumerator/denumerator/denumerator/denumerator)* $
											(error[absorption_feature_2803[p]]*error[absorption_feature_2803[p]]))
	...
	for ind = 0, n_elements(pass_index) - 1 do begin		
		...
		;	get an array of the ratios of the flux
			local_index_2796 = findel(structure[local_index].X_MIN_2796, lambda)
			absorption_feature_2796 =uint(local_index_2796  + dindgen(structure[ind].NUM_PIXELS ))

			local_index_2803 = findel(structure[local_index].X_MIN_2796*2803.5314853/2796.3542699, lambda)
			absorption_feature_2803 =uint(local_index_2803   + dindgen(structure[ind].NUM_PIXELS ))
		
		
			ratio_error_array = fltarr(2, n_elements(absorption_feature_2796))
