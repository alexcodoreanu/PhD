...
function select_c4_systems, lambda, flux, error, minimum, maximum, structure_name
	...
	for ind = 0, n_elements(structure.REDSHIFT) - 1 do begin
		;	get the pixel location in the lambda array
			local_index_1548 = findel(structure[ind].X_MIN_1548, lambda)
			absorption_feature_1548 =uint(local_index_1548  + dindgen(structure[ind].NUM_PIXELS ))

			local_index_1550 = findel(structure[ind].X_MIN_1548*1550.77845/1548.2049, lambda)
			absorption_feature_1550 =uint(local_index_1550  + dindgen(structure[ind].NUM_PIXELS ))
			
			;print, ind
			
		; check that the ratio criteria holds for each pixel 
			check = 0.0
			for p = 0, n_elements(absorption_feature_1548) - 1 do begin
				numerator = (1.0 - flux[absorption_feature_1548[p]])
				if numerator lt 0.0 then numerator = flux[absorption_feature_1548[p]]
			
				denumerator = (1.0 - flux[absorption_feature_1550[p]])
				if denumerator lt 0.0 then denumerator = flux[absorption_feature_1550[p]]
			
				error_value = sqrt((1./denumerator/denumerator)*(error[absorption_feature_1548[p]]*error[absorption_feature_1548[p]])* $
											(numerator*numerator/denumerator/denumerator/denumerator/denumerator)* $
											(error[absorption_feature_1550[p]]*error[absorption_feature_1550[p]]))
	...
	for ind = 0, n_elements(pass_index) - 1 do begin		
		...
		;	get an array of the ratios of the flux
			local_index_1548 = findel(structure[local_index].X_MIN_1548, lambda)
			absorption_feature_1548 =uint(local_index_1548  + dindgen(structure[ind].NUM_PIXELS ))

			local_index_1550 = findel(structure[local_index].X_MIN_1548*1550.77845/1548.2049, lambda)
			absorption_feature_1550 =uint(local_index_1550   + dindgen(structure[ind].NUM_PIXELS ))
		
		
			ratio_error_array = fltarr(2, n_elements(absorption_feature_1548))
