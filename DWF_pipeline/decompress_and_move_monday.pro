...
function decompress_and_move_monday, tar_name
	
	;push_path = "/lustre/projects/p025_swin/tap/cals_forigor/"
	;tar_name = "DECam_00503091.tar"
	
	push_path = "/lustre/projects/p025_swin/pipes/DWF_PIPE/CTIO_PUSH/"
	...
	;	clean up the folder of .sh.o and .sh.e files
		spawn, "rm -rf *.sh.*"
	...
	;	get the folder name	which is the tar_name minus the ".tar"
		...
		jp2_name = strmid(strtrim(tar_name,1),0,string_length - 4);	get the folder name	which is the tar_name minus the ".tar"
		string_length = strlen(tar_name)
		folder_name = strmid(strtrim(tar_name,1),0,string_length - 4) + "/"
		jp2_name = strmid(strtrim(tar_name,1),0,string_length - 4)
		
	; 	make directory to move the .tar ball to
	...
	;	and put the output in jp2_files_list
		...
		untar_ball = "tar -xvf " + tar_name 
		spawn, untar_ball
		
	
		mv_jp2_cmd = "mv " + jp2_name + "*jp2" + " " +  strmid(strtrim(tar_name,1),0,string_length - 4)
		spawn, mv_jp2_cmd
		
		mv_jp2_cmd = "mv " + jp2_name + "*jp2" + " " + folder_name
		spawn, mv_jp2_cmd
		mv_jp2_cmd = "mv " + jp2_name + "*jp2" + " " + folder_name
		spawn, mv_jp2_cmd
		mv_jp2_cmd = "mv " + jp2_name + "*jp2" + " " + folder_name
	...
	;	now read in the jp2_files_list and figure out how many .jp2 files there are
		...
		if n_elements(jp2_files_list) lt 58 then begin
			
			print, ""
			print, "There should be 58 CCD fles for CCD 1 through CCC 59."
			print, "Most likely the tar ball is still being transfered."
			print, "This program will now quit. Try the same command in a couple of minutes."
			exit
	...
	new_shell_directory_name = folder_path + 'shell_scripts'
    new_directory_command = "mkdir " + new_shell_directory_name
	
	spawn, new_directory_command
			print, ""
			print, ""
			print, "The directories:  ", new_shell_directory_name, ' +/shell_scripts'
				...
				full_fits_file_name = new_directory_name + fits_file_name
					...
					;	this will be its' own stand alone IDL code
						...
						;	start the idl bit
							...
							create_new_name	= "new_name = '" + output_path + "ut" + date_string +"/" +  CCD_number + "/' + object + '.' + filter + '" + $
							...
							cd_into_raw_data = "cd " + output_path + "ut" + date_string +"/" +  CCD_number + "/"
									printf, lun, cd_into_raw_data
									
									
							chmod_command = "chmod 777 * "
									printf, lun, chmod_command
									printf, lun, ""
									printf, lun, ""	
									
									
							cd_into_pipeloop_submit = "cd /lustre/projects/p025_swin/pipes/DWF_PIPE/PIPE_SUBMIT/ut" + date_string
									printf, lun, cd_into_pipeloop_submit
									printf, lun, ""
									printf, lun, ""	
									
									
							submit_pipeloop_command = "qsub submit_pipe_" + CCD_number + ".sh"	
									printf, lun, submit_pipeloop_command
									printf, lun, ""
									printf, lun, ""		
	...
	;	check the number of jobs running
		...
		readcol, 'list', l, format=('A'), /silent
			...
			while n_elements(l) gt 15.0 do begin
			...
			endwhile
		wait, 30
		chmod_cmd = "chmod 777 " + output_path + 'ut' + date_string + '/*' 
		spawn, mv_tar_ball_cmd
		...
		print, ''
		print, ''
		print, tar_name, ' finished untaring.' 
		print, 'The files have been decompressed and their name have been adjusted to'
		print, 'reflect the OBJTYPE and FILTER tags. They  are now in the raw data directory'
	...
	;	move tar ball to the folder
		mv_tar_ball_cmd = "mv " + tar_name + " " + folder_path
		spawn, mv_tar_ball_cmd
			

	;	chmod the output the directory
	chmod_cmd = "chmod 777 " + output_path + 'ut' + date_string + '/*' 
	spawn, mv_tar_ball_cmd
