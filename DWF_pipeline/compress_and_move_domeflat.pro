...
function compress_and_move_domeflat, tar_name
	...
	;	untar the file
	;	untar_commasnd = "tar -xf " + tar_name
	;	spawn, untar_command
					...
					;	this will be its' own stand alone IDL code
						...
						;	start the idl bit
							...
							copy_the_old_fits_to_rawdata_location_with_new_name = "cp_command = 'cp -f' + fits_file + ' ' + new_name"		
									...
									read_in_fits_with_header = "file = mrdfits(fits_file,0, header)"
									modify_fits_header = "SXADDPAR, header, 'OBSTYPE', 'dome flat'"
									write_new_fits_file = "mwrfits, fits, fits_file, header,  /create"
										printf, lun, read_in_fits_with_header
										printf, lun, modify_fits_header	
										printf, lun, write_new_fits_file						
										printf, lun, ""
										printf, lun, ""
							
							
									copy_the_old_fits_to_rawdata_location_with_new_name = "cp_command = 'cp ' + fits_file + ' ' + new_name"		
											printf, lun, copy_the_old_fits_to_rawdata_location_with_new_name
											printf, lun, "spawn, cp_command"
											printf, lun, ""
											printf, lun, ""
							...
							pipe_loop_command = ''
							if check_pipe_yes ge 0.0 then pipe_loop_command = "pipeloop = "
							if check_pipe_no ge 0.0 then pipe_loop_command = "; pipeloop will not be used"
									printf, lun, pipe_loop_command
									printf, lun, ""
									printf, lun, ""	
